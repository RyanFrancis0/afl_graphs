...
if it applies to all clubs?), then maybe look at graphing the ladder round by round, that would provide a lot
...
for i in tables[1 + this_season - year_started:]:
    finals_lookout = True
    rows = i.findAll('tr')
...
if FINALS_SOMETHING:
    ...
    for i in teams:
        ...
        for index, k in enumerate(x):
            idx += 1
            a = k < year_began or k > teams[i][0][-1]
            if a or teams[i][0][idx - start_diff] != x[index]:
                if not a:
                    idx = idx - 1
                y.append(z)
                continue
            if finals_cutoff[index] >= teams[i][1][idx - start_diff]:
                z += 1
            y.append(z)
        if y[-1] > maxy:
            maxy = y[-1]
        ax.step(x, y, where='post', label="{}".format(i))
    ax.set_xticks([i for i in range(year_started - year_started % 10, (last_season + (last_season % 10) + 10), 10)])
    ax.set_yticks([i for i in range(0, maxy + 10 - maxy % 10, 10)])
    plt.ylabel('Played Finals')
    plt.xlabel('Years')
    plt.legend()
    plt.title("Finals Played")
    ax.minorticks_on()
    ax.grid(which='minor')
    ax.grid(which='major', color="black")
